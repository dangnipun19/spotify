    ...
    <section className='p-4 space-y-8 pr-8'>
      <div className='flex space-x-2 items-center justify-between'>
        ...
        <div className='flex items-center space-x-4 border-2 border-[#262626] rounded-full h-12 py-3 px-4'>
          <HiOutlineShieldCheck className="text-[#ccc] text-xl"></HiOutlineShieldCheck>
          <MdOutlineSettings className="text-[#ccc] text-xl"></MdOutlineSettings>
          <BiBell className="text-[#ccc] text-xl"></BiBell>
        </div>
        {/* {Profile} */}
        <Dropdown />
      </div>
      ...
      <div className="bg-[#0d0d0d] border-2 border-[#262626] p-4 rounded-xl space-y-4">
        <div className="flex items-center justify-between">
          ...
          <HiViewGrid className="text-[#686868]"></HiViewGrid>
        </div>
        <div className="space-y-4 overflow-y-scroll overflow-x-hidden h-[250px] md:h-[400px] scrollbar-hide">{recentlyPlayed.map((track, index) => (
            <RecentlyPlayed
              key={index}
              track={track}
